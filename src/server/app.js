...
app.post('/', (req, res) => {
    ...
    conn.query(query, [email, password], (err, result) => {
        ...
        if (result.length > 0) {
            ...
            res.status(200).send({ message: '1', id: user_id,username:email });
        } else {
...
app.post('/sidebarKontrol',(req,res)=>{
    ...
    conn.query(kontrolQuery, [id], (err, result) => {
        ...
        if (result.length > 0) {
            ...
            conn.query(query, [ id, isim, soyisim, cinsiyet, dTarihi, email, telefonKodu, telefon, adres, uyruk, kimlikno, universite, bolum, ortalama, mezuniyetDurumu, mTarihi, engelDurumu, engelAcikla, cv, niyetmektubu, pasaport, ikametgah, diploma, ingyetbelgesi], (err, result) => {
                ...
                res.status(200).send({ message: "Basvuru nuz alınmıştır" });
            });
        }
...
app.post('/formGuncelle', (req, res) => {
    ...
    conn.query(kontrolQuery, [id], (err, result) => {
        ...
        if (result.length > 0) {
            ...
            const query = "UPDATE basvuru SET isim = ?, soyisim = ?, cinsiyet = ?, dTarihi = ?, email = ?, telefonKodu = ?, telefon = ?, adres = ?, uyruk = ?, kimlikno = ?, universite = ?, bolum = ?, ortalama = ?, mezuniyetDurumu = ?, mTarihi = ?, engelDurumu = ?, engelAcikla = ?, cv = ?, niyetmektubu = ?, pasaport = ?, ikametgah = ?, diploma = ?, ingyetbelgesi = ? WHERE basvuran_id = ?";
            
            


            conn.query(query, [ isim, soyisim, cinsiyet, dTarihi, email, telefonKodu, telefon, adres, uyruk, kimlikno, universite, bolum, ortalama, mezuniyetDurumu, mTarihi, engelDurumu, engelAcikla, cv, niyetmektubu, pasaport, ikametgah, diploma, ingyetbelgesi,id], (err, result) => {
                if (err) {
                    console.error("Veritabanina başvuru bilgilerini güncellerken hata olustu. ", err);
                    res.status(500).send({ error: "Veritabanina başvuru bilgilerini güncellerken hata olustu." });
                    return;
                }

                res.status(200).send({ message: "Basvurunuz guncellenmiştir" });
            });
        } else {
            res.status(201).send({ error: "Başvurunuz bulunmamaktadır" });
            return;
